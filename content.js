	...
	if (CN_CURRENT_MESSAGE && CN_CURRENT_MESSAGE.length) {
		...
		if (newSentences != null && newSentences.length != CN_CURRENT_MESSAGE_SENTENCES.length) {
			...
			for (i = nextRead; i < newSentences.length; i++) {
				...
				jQuery("#finalscript_bot").html(lastPart);

				if(lastPart.startsWith("An error occurred")){
					location.reload();
				}
...
function CN_StartSpeechRecognition() {
	...
	CN_SPEECHREC.onstart = () => {
		...
		jQuery("#finalscript_bot").html("I'm listening");
	};
	CN_SPEECHREC.onend = () => {
		// Make border grey again
		$("#TTGPTSettings").css("border", "2px solid #888");
		...
		jQuery("#finalscript_bot").html("I've stopped listening");
	};
	...
	CN_SPEECHREC.onresult = (event) => {
		...
		for (let i = event.resultIndex; i < event.results.length; ++i) {
			if (event.results[i].isFinal){ 
				...
				jQuery("#finalscript").html(final_transcript);
			}
		}
		console.log("You have said the following words: "+final_transcript);
		jQuery("#finalscript").html("You have said the following words: "+final_transcript);
...
//stop listening
function CN_OnStopListeningClick(){
	$("#TTGPTSettings").css("border-bottom", "8px solid red");
	console.log("Đã nhấn dừng nghe");
	CN_SPEECHREC.stop();
}

function CN_OnStartListeningClick(){  
	console.log("Đã nhấn bắt đầu nghe");
	CN_StartSpeechRecognition();
	$("#TTGPTSettings").css("border-bottom", "8px solid green");

	jQuery("#finalscript_bot").html("Hãy bắt đầu hỏi!")
}
...
function CN_ToggleButtonClick() {
	...
	switch(action) {
		...
		case "stoplistening":
			CN_OnStopListeningClick();
			return;
		case "startlistening":
			CN_OnStartListeningClick();
			return;
		// Open settings menu
		case "settings":
...
function CN_StartTTGPT() {
	CN_SayOutLoud("Xin chào!");
	jQuery("#finalscript").html("Xin chào!");
	CN_FINISHED = false;
...
function CN_InitScript() {
	...
	jQuery("body").append("<span style='position: fixed; top: 8px; right: 16px; display: inline-block; " +
		...
		"<br/>"+
		"<button class='CNToggle' title='Stop listen' data-cn='startlistening'> &#9199; </button> " + // start listen
		"<br/>"+
		"<button class='CNToggle' title='Stop listen' data-cn='stoplistening'>&#9209; </button> " + // stop listen
		"<hr/>"+ 
		"<p style='font-size:12px; color:white' id='finalscript'></p>"+
		"<hr/>"+ 
		"<p style='font-size:12px; color:yellow' id='finalscript_bot'></p>"+
		"</span></span>");
